...
scene.fog = new THREE.Fog(0x241f18, 20, 70);
scene.background = new THREE.Color(0x241f18);
...
// ---------- lighting ----------
scene.add(new THREE.AmbientLight(0x6a5c48, 1.3));
const torchLight = new THREE.PointLight(0xff9a3d, 2.1, 22);
torchLight.position.set(0, 4, 4);
scene.add(torchLight);
const moonLight = new THREE.DirectionalLight(0x8899cc, 0.65);
moonLight.position.set(-5, 10, -10);
scene.add(moonLight);
const fillLight = new THREE.DirectionalLight(0xffd9a0, 0.25);
...
function makeGroundSegment(zPos){
  ...
  const floorMat = new THREE.MeshStandardMaterial({ color:0x453a2c, roughness:.95 });
  ...
  for(const side of [-1,1]){
    const pillar = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color:0x362c1f, roughness:.9 })
    );
